...
public class CollectionViewWaterfallLayout: UICollectionViewLayout {
    ...
    public var columnCount:Int = 2 {
        didSet {
            invalidateIfNotEqual(oldValue: oldValue as NSNumber, newValue: columnCount as NSNumber)
        }
    }
    public var minimumColumnSpacing:Float = 10.0 {
        didSet {
            invalidateIfNotEqual(oldValue: oldValue as NSNumber, newValue: minimumColumnSpacing as NSNumber)
        }
    }
    public var minimumInteritemSpacing:Float = 10.0 {
        didSet {
            invalidateIfNotEqual(oldValue: oldValue as NSNumber, newValue: minimumInteritemSpacing as NSNumber)
        }
    }
    public var headerHeight:Float = 0.0 {
        didSet {
            invalidateIfNotEqual(oldValue: oldValue as NSNumber, newValue: headerHeight as NSNumber)
        }
    }
    public var footerHeight:Float = 0.0 {
        didSet {
            invalidateIfNotEqual(oldValue: oldValue as NSNumber, newValue: footerHeight as NSNumber)
        }
    }
        ...
        for section in 0..<numberOfSections! {
            ...
            */
            ...
            for idx in 0..<itemCount {
                ...
                let itemSize = delegate?.collectionView(collectionView: collectionView!, layout: self, sizeForItemAtIndexPath: indexPath) ?? CGSize.zero
                var itemHeight: Float = 0.0
                if itemSize.height > CGFloat(0) && itemSize.width > 0.0 {
